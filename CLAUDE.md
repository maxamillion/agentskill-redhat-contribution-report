...
Heavy computation (workflow detection, PR analysis with verification, governance file scanning, batch username resolution, report validation) is offloaded to standalone Python scripts in `assets/`. Sub-agent prompts reference these scripts by path (`{assets_dir}/script.py`), keeping prompts lean and minimizing context window usage. A final auditor sub-agent validates the generated report against checkpoint data and live GitHub API spot-checks before cleanup.
